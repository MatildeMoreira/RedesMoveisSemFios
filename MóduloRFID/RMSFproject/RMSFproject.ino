/**********************************************************************************
 * Controlo de Entrada e Saida de Pessoas numa instalacao critica
 *
 * Disciplina: Redes Móveis e sem fios
 * 
 * Trabalho realizado por: Joao Cardoso, nº84096
 *                         Matilde Moreira, nº84137
 *
 * Docente: António Manuel Grilo
 *
 * MIFARE RFID card que usa leitor RFID-RC522 
 * ---------------------------------
 *             MFRC522      Arduino 
 *             Reader/PCD   Uno 
 * Signal      Pin          Pin     
 ...
 * SPI SCK     SCK           13
 *
 * Consultar: https://github.com/miguelbalboa/rfid
 * 
 * Modulo Camara OV7670 CMOS VGA SCSB    
 * 
 *        
 ************************************************************************************/
...
void setup() {
  Serial.begin(9600); // Inicializa as comunicações em serie com o PC
  SPI.begin(); //Inicia o bus SPI
  
  mfrc522.PCD_Init(); // Inicia o cartao MFRC522
  Serial.println(F("Leitura e Escrita no cartao MIFARE PICC:"));    //shows in serial that it is ready to read
  delay(2000);
...
void loop() {
  ...
  Serial.println(F("**Cartao Detetado**"));

  mfrc522.PICC_DumpDetailsToSerial(&(mfrc522.uid)); //dump some details about the card
  //mfrc522.PICC_DumpToSerial(&(mfrc522.uid));      //uncomment this to see all blocks in hex

  Serial.print(F("Nome do Trabalhador: "));
  ...
  status = mfrc522.MIFARE_Read(blockAddr,nome,&len);

  if ( status != MFRC522::STATUS_OK){
    Serial.println("\n");    
    Serial.print(F("[ERRO] Leiura: "));
    Serial.println(mfrc522.GetStatusCodeName(status));
    return;
  }
  ...
  status = mfrc522.MIFARE_Read(blockAddr,apelido,&len);
  ...
  for (uint8_t i=0; i<16; i++)
  {
      Serial.write(apelido[i]);
  }

  //----------------------------------------------------------------------
  //captureImg(640, 320);
  Serial.println(F("\n** Fim da Leitura **\n"));
  delay(1000); // alterar valor se se quiser ler os cartoes mais rapidamente
  mfrc522.PICC_HaltA();
